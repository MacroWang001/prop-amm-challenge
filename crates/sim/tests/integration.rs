...
fn test_normalizer_basic_execution() {
    ...
    let output = exec.execute(0, f64_to_nano(10.0), rx, ry, &EMPTY_STORAGE).unwrap();
    let output_f64 = nano_to_f64(output);
    assert!(output_f64 > 0.09 && output_f64 < 0.11, "buy output: {}", output_f64);

    let output = exec.execute(1, f64_to_nano(1.0), rx, ry, &EMPTY_STORAGE).unwrap();
    let output_f64 = nano_to_f64(output);
...
fn test_normalizer_math_correctness() {
    ...
    let output = exec
        .execute(0, f64_to_nano(100.0), f64_to_nano(100.0), f64_to_nano(10000.0), &EMPTY_STORAGE)
        .unwrap();
    let output_f64 = nano_to_f64(output);
...
fn test_starter_has_higher_fee() {
    ...
    let start_out = exec_start.execute(0, input, rx, ry, &EMPTY_STORAGE).unwrap();
...
fn test_monotonicity() {
    ...
    let mut prev = 0u64;
    for &size in &sizes {
        let out = exec.execute(0, f64_to_nano(size), rx, ry, &EMPTY_STORAGE).unwrap();
...
fn test_convexity() {
    ...
    for &size in &sizes {
        let out_lo = nano_to_f64(exec.execute(0, f64_to_nano(size), rx, ry, &EMPTY_STORAGE).unwrap());
        let out_hi = nano_to_f64(exec.execute(0, f64_to_nano(size + eps), rx, ry, &EMPTY_STORAGE).unwrap());
...
fn test_after_swap_noop() {
    ...
    exec.execute_after_swap(0, 1000, 500, 2000, 3000, &mut storage).unwrap();
...
fn test_storage_persists_across_swaps() {
    ...
    let config = SimulationConfig { n_steps: 100, seed: 99, ..SimulationConfig::default() };
    let result = prop_amm_sim::engine::run_simulation(load_starter(), load_normalizer(), &config).unwrap();
...
fn test_storage_reset_between_simulations() {
    ...
    let config = SimulationConfig { n_steps: 500, seed: 42, ..SimulationConfig::default() };
    let result1 = prop_amm_sim::engine::run_simulation(load_starter(), load_normalizer(), &config).unwrap();
    let result2 = prop_amm_sim::engine::run_simulation(load_starter(), load_normalizer(), &config).unwrap();
    assert_eq!(result1.submission_edge, result2.submission_edge,
